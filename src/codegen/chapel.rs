...
fn write_config_constants(src: &mut String, manifest: &Manifest) -> Result<()> {
    ...
    writeln!(src, "  // ------------------------------------------------------------------")?;
    writeln!(src, "  // Runtime configuration (override with --name=value)")?;
    writeln!(src, "  // ------------------------------------------------------------------")?;
    writeln!(src)?;
...
fn write_ffi_declarations(src: &mut String, _safe_name: &str) -> Result<()> {
    writeln!(src, "  // ------------------------------------------------------------------")?;
    writeln!(src, "  // FFI declarations (implemented in Zig bridge / user code)")?;
    writeln!(src, "  // ------------------------------------------------------------------")?;
    writeln!(src)?;
    ...
    // Data I/O
    writeln!(src, "  // Data I/O: load/store serialised items on locale 0")?;
    writeln!(
        src,
...
fn write_helpers(src: &mut String, manifest: &Manifest) -> Result<()> {
    ...
    writeln!(src, "  // Compute the item range assigned to a locale for even distribution.")?;
    writeln!(
        src,
    ...
    if manifest.workload.partition == "chunk" {
        writeln!(
            src,
            "  // Compute the item range for a given chunk index."
        )?;
        writeln!(
            src,
        ...
        writeln!(
            src,
            "    const lo = chunkIdx * grainSize;"
        )?;
        writeln!(
            src,
            "    const hi = min(lo + grainSize, nItems);"
        )?;
        writeln!(src, "    return lo..#(hi - lo);")?;
        writeln!(src, "  }}")?;
        writeln!(src)?;
...
fn write_distribution_phase(src: &mut String, manifest: &Manifest) -> Result<()> {
    ...
    match manifest.workload.partition.as_str() {
        ...
        other => {
            writeln!(
                src,
                "    // ERROR: unknown partition strategy '{other}'"
            )?;
            writeln!(
                src,
                "    halt(\"Unknown partition strategy: {other}\");"
            )?;
        }
    }
...
fn write_chunk_distribution(src: &mut String) -> Result<()> {
    ...
    writeln!(
        src,
        "        const cr = chunkRange(chunkIdx, nItems);"
    )?;
    writeln!(
        src,
...
fn write_process_single_item(src: &mut String, idx_var: &str) -> Result<()> {
    ...
    writeln!(
        src,
        "          resultData[{idx_var}] = localOut;"
    )?;
    writeln!(
        src,
        "          resultSizes[{idx_var}] = outLen;"
    )?;
    writeln!(
        src,
        "          resultOk[{idx_var}] = true;"
    )?;
    writeln!(src, "        }} else {{")?;
    writeln!(
        src,
...
fn write_gather_phase(src: &mut String, manifest: &Manifest) -> Result<()> {
    ...
    match manifest.workload.gather.as_str() {
        ...
        other => {
            writeln!(
                src,
                "    // ERROR: unknown gather strategy '{other}'"
            )?;
            writeln!(
                src,
                "    halt(\"Unknown gather strategy: {other}\");"
            )?;
        }
    }
...
fn write_gather_reduce(src: &mut String) -> Result<()> {
    ...
    writeln!(
        src,
        "    var accumLen: c_size_t = resultSizes[0];"
    )?;
    writeln!(src)?;
    writeln!(src, "    on Locales[0] {{")?;
    ...
    writeln!(
        src,
        "        var tmpBuf: [0..#maxItemBytes] uint(8);"
    )?;
    writeln!(
        src,
        "        var tmpLen: c_size_t = 0;"
    )?;
    writeln!(
        src,
        "        const rc = c_reduce("
    ...
    writeln!(
        src,
        "    // Store reduced result as item 0"
    )?;
    writeln!(
        src,
        "    resultData[0] = accumBuf;"
    )?;
    writeln!(
        src,
        "    resultSizes[0] = accumLen;"
    )?;
    writeln!(
        src,
        "    resultOk[0] = true;"
...
fn write_gather_tree_reduce(src: &mut String) -> Result<()> {
    ...
    writeln!(
        src,
        "    var localeResultLen: [0..#numLocales] c_size_t;"
    )?;
    writeln!(
        src,
        "    var localeHasResult: [0..#numLocales] bool;"
    )?;
    writeln!(src)?;
    ...
    writeln!(
        src,
        "      var localAccum: [0..#maxItemBytes] uint(8);"
    )?;
    writeln!(
        src,
        "      var localAccumLen: c_size_t = 0;"
    )?;
    writeln!(src)?;
    writeln!(src, "      for i in myRange {{")?;
    writeln!(
    ...
    writeln!(
        src,
        "          localAccum = resultData[i];"
    )?;
    writeln!(
        src,
        "          localAccumLen = resultSizes[i];"
    )?;
    ...
    writeln!(
        src,
        "      localeResultBuf[loc.id] = localAccum;"
    )?;
    writeln!(
        src,
        "      localeResultLen[loc.id] = localAccumLen;"
    )?;
    writeln!(
        src,
        "      localeHasResult[loc.id] = !first;"
    )?;
    writeln!(src, "    }}")?;
    writeln!(src)?;
    ...
    writeln!(
        src,
        "    // Now tree-reduce across locale accumulators"
    )?;
    writeln!(
        src,
    ...
    writeln!(
        src,
        "        if localeHasResult[partner] {{"
    )?;
    writeln!(
        src,
        "          if !localeHasResult[i] {{"
    )?;
    writeln!(
        src,
    ...
    writeln!(
        src,
        "            localeResultBuf[i] = tmpBuf;"
    )?;
    writeln!(
        src,
        "            localeResultLen[i] = tmpLen;"
    )?;
    writeln!(src, "          }}")?;
    writeln!(src, "        }}")?;
    writeln!(src, "      }}")?;
    ...
    writeln!(
        src,
        "    // Store tree-reduced result as item 0"
    )?;
    writeln!(
        src,
        "    resultData[0] = localeResultBuf[0];"
    )?;
    writeln!(
        src,
        "    resultSizes[0] = localeResultLen[0];"
    )?;
    writeln!(
        src,
        "    resultOk[0] = localeHasResult[0];"
    )?;
    writeln!(
        src,
...
fn write_gather_first(src: &mut String) -> Result<()> {
    ...
    writeln!(
        src,
        "      writeln(\"  Found match at index \", foundIdx);"
    )?;
    writeln!(src, "    }} else {{")?;
    writeln!(
        src,
...
fn write_store_phase(src: &mut String, manifest: &Manifest) -> Result<()> {
    writeln!(src, "    // ================================================================")?;
    writeln!(src, "    // Phase 4: Store results via c_store_result on locale 0")?;
    writeln!(src, "    // ================================================================")?;
    writeln!(src)?;
    ...
    match manifest.workload.gather.as_str() {
        ...
        "first" => {
            ...
            writeln!(
                src,
                "      if foundIdx >= 0 && resultOk[foundIdx] {{"
            )?;
            writeln!(
                src,
